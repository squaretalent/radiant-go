    ...
    class Radiant
      ...
      def bootstrap
        Dir.chdir(@name) do
          ...
          command = "rake db:bootstrap OVERWRITE=true ADMIN_NAME=#{Config.admin_name} ADMIN_USERNAME=#{Config.admin_user} ADMIN_PASSWORD=#{Config.admin_pass} DATABASE_TEMPLATE=#{Config.database_template}"
          begin
            %x[#{command}]
          rescue
            %x[#{command}]
